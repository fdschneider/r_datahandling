...
```{r, eval = FALSE}
  ...
  d1 <- read.csv("data/12807/12807.txt")
```
```{r, echo = FALSE}
...
Sometimes a downstream code or function expects a column to have a certain name, or you want to fix a typo in the original column header. 
...
You may want to keep your data table tidy and strip all columns that are not needed for your analysis. There are a couple of ways of deleting unwanted columns.
...
Obviously, the genus "Achemilla" does not exist. 

```{r}
subset(d1, plant_genus %in% c("Alchemilla", "Achemilla"))
```

What we need is to reunite the factor level 'Achemilla' with 'Alchemilla'. The graceful solution requires `revalue` from package `plyr`: 
...
2. fix date format 
3. rename exploratory factor levels to "HAI", "ALB" and "SCH" (in this order)
...
```{r, echo = FALSE}
  ...
  #3.
  d1$exploratory <- factor(d1$exploratory, levels = c("Hainich", "SchwaebischeAlb", "Schorfheide"), labels = c("HAI", "ALB", "SCH"))
  d2$exploratory <- factor(d2$exploratory, levels = c("Hainich", "SchwaebischeAlb", "Schorfheide"), labels = c("HAI", "ALB", "SCH"))
...
Once you have read-in a data table and fixed the obvious errors, you mostly need only some of the columns to do your further calculations. Most beginners guides describe `attach()` and `detach()` to access data within a table. 
...
This allows us to create very compact data extracts. 
...
We continue to work with the data the Land Use Intensity Index.  
